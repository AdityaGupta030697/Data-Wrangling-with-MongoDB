...
def main():
    ...
    #pretty_print(results_oned)
    #print("One Direction was formed in :", results_oned["artists"][0]["life-span"]["begin"])

    # Query for information in the database about bands named Beatles
    #results_b = query_by_name(ARTIST_URL, query_type["simple"], "Beatles")
    # pretty_print(results_b)
    
    #for alias in results_b["artists"][0]["aliases"]:
        #if alias["locale"] == "es":
            #print("The Spanish alias for Beatles is " + alias["name"])
            
    # Query for information in the database about bands named Queen
    #results_q = query_by_name(ARTIST_URL, query_type["simple"], "Queen")
    #pretty_print(results_q["artists"][0]["begin-area"]["name"])
    ...
    #pretty_print(results_fak)

    count=0
    for artist in results_fak["artists"]:
        if artist["name"] == "First Aid Kit":
            count=count+1
        
    #print("No of Bands named First Aid Kit:",count)
