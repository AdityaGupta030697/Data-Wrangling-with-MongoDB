...
def article_overview(kind, period):
    ...
    for it,val in enumerate(data):
        temp = {data[it]["section"].encode('ascii'):data[it]["title"].encode('ascii', 'ignore')}
        titles.append(temp)
        #print(data[it]["url"])
        for it2 in data[it]["media"]:
            for it3 in it2["media-metadata"]:
                if it3["format"] == "Standard Thumbnail":
                    urls.append(str(it3["url"]))
                    break
    #print(urls)    
    return (titles, urls)
