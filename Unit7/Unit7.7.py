...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for i in range(3):
            next(reader)
        classification_list = ['class_label','family_label','phylum_label','order_label','genus_label','kingdom_label']
        for line in reader:
            temp = {}
            temp2 = {}
            for f,v in FIELDS.items():
                if(f == 'rdf-schema#label'):
                    regex = re.findall('\(\w+\)',line[f])
                    if len(regex) != 0:
                        temp[v] = line[f].replace(regex[0],"").strip()
                    else:
                        temp[v] = line[f].strip()
                elif (f == 'name'):
                    if(line[f]== 'NULL'):
                       temp[v] = line['rdf-schema#label']
                    else:
                        temp[v] = line[f].strip()
                elif (f == 'synonym') and (line[f]!='NULL'):
                    y = line[f].replace('{*',"").replace("}","").strip().split('|* ')
                    temp[v] = y
                elif (f in classification_list):
                    if(line[f]=='NULL'):
                        temp2[v] = None
                    else:
                        temp2[v] = line[f]
